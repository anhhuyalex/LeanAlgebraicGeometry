...
\begin{proof}
  ...
  G \times G\) and composing on the right with \((\mathrm{id} \times \mathrm{inv})\) then
  \(m\) gives \(\Phi\). Its domain is at least \(U \times U \subseteq X \times X\), which
  is dense because \(U\) is dense in \(X\).
