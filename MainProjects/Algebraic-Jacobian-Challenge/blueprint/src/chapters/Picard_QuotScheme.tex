...
\begin{remark}
  ...
  Establishing that monomorphism fact scheme-theoretically (rather than only
  on the affine charts of that proof) is expected to go through the
  basis-local criterion \cref{lem:modules_mono_of_injective_app_of_isBasis},
  reducing it to injectivity on sections over a trivialising affine cover;
  this reduction is not yet assembled into a closed proof of
  \cref{lem:relative_divisor_base_change} along this route, since the
  needed section-surjectivity input --- vanishing of the first cohomology of
  the kernel module over a trivialising affine open of an \emph{arbitrary}
  (not necessarily affine) ambient scheme --- is not yet available.
\end{remark}
...
\begin{proof}
  ...
  Injectivity is \Cref{lem:serre_twist_forms_embed}; the open surjectivity
  direction (``graded separation'') is: given a compatible away-fraction
  family \((s_i)_i\) satisfying the condition of
  \Cref{lem:serre_twist_away_bridge}, each \(s_i = G_i/X_i^{k_i}\) for some
  homogeneous \(G_i\) of degree \(k_i + m\), and the transition condition
  forces \(k_i \leq m\) for every \(i\) (no pole along any chart boundary)
  and the forms \(G_i \cdot X_i^{m - k_i}\) to agree pairwise as elements of
  the fraction field of \(\mathbb Z[X_i : i \in n]\), hence to be a single
  homogeneous form \(F\) of degree \(m\) with \(F/X_i^m = s_i\) for every
  \(i\). This is the classical computation \(\Gamma(\mathbb P^n,
  \mathcal O(m)) = (\mathbb Z[X_i])_m\); it is not yet formalised.
\end{proof}
