...
\begin{proof}
  ...
  binary tensor product up to natural isomorphism:
  \(f^*(\mathcal{F} \otimes_{\mathcal{O}_X} \mathcal{G}) \cong
  f^*\mathcal{F} \otimes_{\mathcal{O}_Y} f^*\mathcal{G}\) for all
  \(\mathcal{O}_X\)-modules \(\mathcal{F}\), \(\mathcal{G}\).

  For the latter, for every \(\mathcal{O}_Y\)-module \(\mathcal{H}\) there
  are isomorphisms, natural in all three variables:
  \begin{align*}
...
[Base change of the ideal of a relative effective divisor]
  ...
  epimorphism of \(\mathcal O_X\)-modules with \(F\) finitely presented and
  flat over \(S\) (\cref{def:coherent_sheaf_flat}), and suppose the kernel
...
\begin{proof}
  ...
  and \(E\) is an extension of the quasi-coherent \(F\) by the
  quasi-coherent \(\mathcal K\) --- quasi-coherent modules form a weak Serre
  subcategory of all \(\mathcal O_X\)-modules, in particular one closed
  under extensions (Stacks 01LA).
  ...
  O_V\) persists under shrinking), and an affine open
  ...
  \[
    \operatorname{Tor}_1^R\bigl(\Gamma(F, V), R'\bigr)
      ...
      \to 0,
  \]
  and the Tor term vanishes because \(\Gamma(F, V)\) is \(R\)-flat
  (\cref{def:coherent_sheaf_flat} at the affine pair \((U, V)\)).  By the
  chart identification, this says that
...
[Relative-divisor functor]
  ...
  \uses{def:div_family, lem:relative_divisor_base_change,
    lem:pullback_finite_presentation, lem:coherent_flat_base_change,
    lem:proper_support_base_change}
  \source{kleiman-picard}
  ...
  kernel of the pulled-back quotient
  (\cref{lem:relative_divisor_base_change}).  Functoriality holds because
...
The whole chain is proved except for one leaf: the \(H^0\)
flat-base-change heart over a compatible affine pair
(\cref{thm:quot_pullback_baseMap_sectionEquiv_qc}, Stacks tag 02KE),
from which every isomorphism statement below follows by the affine-pair
