...
\begin{proof}
  ...
  statement used to build \(\functionFieldIso\)
  ...
  \(\mathrm{HasDedekindChart}\, C\), discharged by \(\inferInstance\)
...
[Finite support of the order function]
  ...
  \lean{AlgebraicGeometry.Adelic.finite_support_order}
  \uses{thm:adelic_chartRing_isDedekindDomain, def:adelic_pointValuation, lem:order_eq_order_restrict}
  % [scheidler] p. 31 (deg div(z) = 0, finiteness of zeros/poles); [papaioannou] Cor. 1.12
  \source{scheidler-function-fields, papaioannou-algebraic-rr}
  For \(f \in K^\times\) the set
  \(\{ P \text{ prime divisor} : v_P(f) \ne 0\}\) is finite. In
  particular the principal divisor \(\operatorname{div} f = \sum_P v_P(f)\,P\)
  is a well-defined Weil divisor; this discharges the finite-support
  obligation \Cref{lem:rationalMap_order_finite_support}.
\end{lemma}
\begin{proof}
  Cover \(C\) by the two affine charts \(U_0, U_1\), each with Dedekind
  coordinate ring \(A_i\) and \(\operatorname{Frac} A_i = K\)
  (\Cref{thm:adelic_chartRing_isDedekindDomain}). Fix \(i\). Writing
  \(f = a/b\) with \(a, b \in A_i \setminus \{0\}\), a height-one prime
  \(\mathfrak p\) of \(A_i\) satisfies \(v_{\mathfrak p}(f) \ne 0\) only
  if \(\mathfrak p\) divides the principal fractional ideal
  \((a)\) or \((b)\); the set of such primes is finite because in a
  Dedekind domain each nonzero element lies in only finitely many
  height-one primes (finiteness of the factorisation support). Under the
  bijection of \Cref{def:adelic_pointValuation} these primes correspond
  to the prime divisors with point in \(U_i\) at which \(v_P(f) \ne 0\).
  As \(C = U_0 \cup U_1\), every prime divisor lies in \(U_0\) or
  \(U_1\), so the full support is contained in the union of two finite
  sets and is finite. Naturality \Cref{lem:order_eq_order_restrict}
  guarantees the two chart computations agree on \(U_0 \cap U_1\).
\end{proof}

\section{Tier 1a --- the cover cokernel and the bridge to \texorpdfstring{\(H^1\)}{H1}}
...
\begin{proof}
  ...
  site and later closed by \(\inferInstance\); every downstream consumer
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item Under the discharged carriers
      ...
      \(\cechToHModuleIso\) (\Cref{def:Scheme_cechToHModuleIso})
...
\section{Tier 1b --- finiteness of the base cokernel via \texorpdfstring{\(\mathbb{P}^1\)}{P1}}
...
[KEYSTONE: the base cokernel is finite-dimensional]
  ...
  \[
    \FiniteDimensional\ k\ \bigl(A_{01}/(A_0 + A_1)\bigr)
    = \FiniteDimensional\ k\ \check{H}^1(0).
  \]
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  \(\FiniteDimensional\ k\ \check{H}^1(0)\); the bridge
...
\section{Tier 2 --- the \texorpdfstring{\(\chi\)}{chi}-ledger}
...
\begin{proof}
  ...
  theorem, discharged later by \(\inferInstance\).
\end{proof}
...
\begin{proof}
  ...
  \(\inferInstance\) once \(\mathrm{HasResiduePairing}\) is discharged.
\end{proof}
