...
Grothendieck topology (absent from Mathlib at the pinned revision) is
needed. Without a section the plain functor is not even a Zariski sheaf in
...
[Line bundle / Quot correspondence via the Abel map]
  ...
  \uses{thm:quot_representable, thm:relative_pic_quotient_well_defined,
        def:rel_pic_sharp, def:has_abel_map,
  ...
  \[
    A_{C/k} \;\colon\; \mathrm{Div}_{C/k} \;\longrightarrow\; \Pic^\sharp_{C/k},
    \qquad D \;\longmapsto\; [\mathcal{O}_{C \times_k T}(D)],
  \]
  where \(\mathrm{Div}_{C/k}(T)\) is the set of relative effective Cartier divisors
  on \(C \times_k T\) flat over \(T\), and the target is the relative Picard
  functor of \cref{def:rel_pic_sharp} (per Kleiman \S 3, Def.~dfn:Abel, ``the
  target \(\Pic_{X/S}\) may be replaced by any of its associated sheaves'';
  under the chapter's standing rational-point hypothesis the plain relative
  functor already coincides with its \'etale sheafification, see
  \cref{sec:fga_pic_setup}). The functor \(\mathrm{Div}_{C/k}\) is representable
...
\begin{proof}
  ...
  \cref{thm:relative_pic_quotient_well_defined} yields the
  natural map \(A_{C/k}(T) : \mathrm{Div}_{C/k}(T) \to \Pic^\sharp_{C/k}(T)\).
\end{proof}
...
\begin{itemize}
  ...
  \item \texttt{AlgebraicGeometry.Scheme.PicScheme.abelMap}
    ...
    \cref{chap:Picard_QuotScheme}, restricts to the open subscheme of
    relative effective divisors via the Kleiman~\S 3 effective-divisor
    open criterion, and composes with the quotient map to
    \(\Pic^\sharp_{C/k}\). (Still carrier-backed: the source
    \(\mathrm{Div}_{C/k}\) awaits the A.2.b engine.)
...
[Existence carrier for the Abel map]
  ...
  transformation of presheaves \(\mathrm{Div}_{C/k} \to \Pic^\sharp_{C/k}\),
  i.e.\ a candidate Abel map from the carrier functor of
  \cref{def:div_functor_carrier} to the (run-0008: now real) relative
  Picard functor of \cref{def:pic_sharp_carrier}.
\end{definition}
...
[Existence carrier for the Picard scheme]
  ...
  \(X\) carrying a representability witness for the relative Picard functor
  \(\Pic^\sharp_{C/k}\) of \cref{def:pic_sharp_carrier}; in other words,
  that the Picard scheme \(\Pic_{C/k}\) exists. It is the existence carrier
  from which \cref{def:pic_scheme} extracts the representing object. Its
  (sole, sorry-carrying) instance \cref{def:inst_has_pic_scheme} is
  conditional on the \(k\)-rational point \cref{def:has_rational_point}.
\end{definition}
...
[Existence witness for the Picard scheme]
  ...
  substrate. See \cref{sec:fga_pic_sorry_closure_order}.
\end{definition}
...
[Existence witness for the representability identification]
  ...
  choice specification says exactly that the chosen scheme represents
  \(\Pic^\sharp_{C/k}\). The FGA content lives entirely upstream in
  \cref{def:inst_has_pic_scheme}.
\end{definition}

\begin{proof}\leanok
  \uses{def:has_pic_scheme}
  By \texttt{Exists.choose\_spec} applied to the existential of
...
[Existence witness for local finiteness]
  ...
  \uses{def:pic_scheme_lft, def:has_rational_point}
  The instance recording that \cref{def:pic_scheme_lft} holds for every
  smooth proper geometrically integral curve \(C/k\) with a \(k\)-rational
  point. A sorry-carrying site (run 0008); the statement is true because
  representing objects are unique up to isomorphism and local finiteness is
  isomorphism-invariant, so Kleiman \S 4 Thm.~th:main(1) transfers from
  Kleiman's construction to the chosen witness of
  \cref{def:has_pic_scheme}. Its discharge is part of the Sorry-A
  obligation (\cref{subsec:sorry_has_pic_scheme}).
\end{definition}

\begin{proof}

\leanok
  Typed statement in Lean; the semantic discharge is part of
...
% superseded by the picSharp rewire. The Lean file now carries THREE open
% sorries, all ⟨sorry⟩ instance bodies; every statement they assert is
% mathematically TRUE under the chapter's standing rational-point
% hypothesis. The historical per-sorry analyses (Sorries 1, 4, 6, 7 of the
...
after the run-0008 rewire, \textbf{four} open sorries, each isolated in a
single \(\langle\)sorry\(\rangle\) instance constructor of a
\texttt{Prop}-valued typeclass, so every extracted definition and theorem
body is itself axiom-clean, and consumers that quantify over the
typeclasses are kernel-clean. The four: \texttt{instHasPicScheme}
(Sorry~A), \texttt{instHasDivFunctor} (Sorry~B), \texttt{instHasAbelMap}
(Sorry~C), and \texttt{instPicSchemeLocallyOfFiniteType}
(\cref{def:inst_pic_scheme_lft}, part of the Sorry-A obligation ---
Kleiman th:main(1)). The closed items of the former accounting: the
relative Picard functor carrier is REAL (\cref{def:pic_sharp_carrier}),
the representability identification is PROVED
(\cref{def:inst_pic_sharp_representable}), the group-scheme structure is
...
(\cref{chap:Picard_QuotScheme}) for \(\mathrm{Div}_{C/k}\) and the Abel
map (Sorries~B, C); (ii) the Route~C Riemann--Roch substrate
...
\subsection{Sorry B --- \texttt{instHasDivFunctor}}
...
\paragraph{Mathematical content.} The asserted \texttt{Nonempty} statement
is trivially true (any functor witnesses it); the sorry is retained only
so that the carrier \(\mathrm{Div}_{C/k}\)
(\cref{def:div_functor_carrier}) is flagged as awaiting its real
definition: the subfunctor of the Hilbert functor
\(\Hilb_{C/k} = \Quot_{\mathcal{O}_C/C/k}\) of relative effective Cartier
divisors, representable by an open subscheme (Kleiman \S 3,
Thm.~th:repDiv). Closes when \cref{chap:Picard_QuotScheme} commits its
pin; until then the extracted carrier is OPAQUE and nothing may rely on
its values.

\subsection{Sorry C --- \texttt{instHasAbelMap}}
...
\paragraph{Mathematical content.} The asserted \texttt{Nonempty} statement
is true (even constructively: the pointwise-zero natural transformation
into the group-valued target is natural), but the intended witness is the
substantive Abel map \(D \mapsto [\mathcal O_{C \times_k T}(D)]\) of
\cref{lem:line_bundle_quot_correspondence}. Closes together with Sorry~B
once the A.2.b engine lands (the run-0008 rewire already fixed the
TARGET to the real relative functor, so only the source side is open).

\subsection{Closure order}
\label{subsec:fga_pic_closure_order_summary}

Dispatch order: \(B \to C \to A\). Sorries B and C are gated on
\cref{chap:Picard_QuotScheme} committing its Quot/Hilbert pin; Sorry~A
additionally needs the Route~C Riemann--Roch substrate and the
Altman--Kleiman/EGA descent inputs, and consumes B and C. Downstream
Route~A consumers proceed under typeclass quantification
(\texttt{[HasPicScheme C]}) and stay kernel-clean; the
