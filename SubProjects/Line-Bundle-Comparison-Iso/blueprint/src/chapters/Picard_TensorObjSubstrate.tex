...
\end{lemma}
\begin{proof}
  \uses{lem:tensorobj_restrict_iso_eq_pullback_tensor_map, lem:pullback_tensor_map_basechange, lem:pullback_tensor_map_natural, lem:trivialisation_uiota_restrict_compat, def:unitrestrictiso, def:restrictcompreindex, lem:image_preimage_of_le}
  This is the \emph{unit} companion of S2, and like S2 it is base-change bookkeeping on Bridge B1.
  The left unitor at the unit, \(\mathtt{tensorObj\_unit\_iso}\), is the unit-evaluation of the same
  base-change tensor comparison whose restriction-naturality is governed by B1
  (\cref{lem:tensorobj_restrict_iso_eq_pullback_tensor_map}): substituting B1 and the unit
  identification \(u_\iota = \mathtt{unitRestrictIso}\,j\) (\cref{def:unitrestrictiso}) expresses
  both the \(U\)- and \(V\)-built contractions as \(\mathtt{restrictFunctorIsoPullback}\)-conjugates
  in the pullback world. There the equation is again the composition law
  \(\mathtt{pullbackTensorMap\_restrict}\) (\cref{lem:pullback_tensor_map_basechange}) for the
  factorisation \(j \mathbin{;} \iota_U = \iota_V\), evaluated at the unit, together with naturality
  of \(\mathtt{pullbackTensorMap}\) (\cref{lem:pullback_tensor_map_natural}); the \(V\)-side unit leg
  is supplied by \(u_\iota\) (\cref{lem:trivialisation_uiota_restrict_compat}). The reindexing
  \(\varrho = \mathtt{restrictCompReindex}\,j\) (\cref{def:restrictcompreindex}) is absorbed exactly
  as in S2 by \(\mathtt{restrictFunctorCongr}\) along \(j \mathbin{;} \iota_U = \iota_V\)
  (\cref{lem:image_preimage_of_le}). No sheaf-level monoidal structure is involved.
\end{proof}
