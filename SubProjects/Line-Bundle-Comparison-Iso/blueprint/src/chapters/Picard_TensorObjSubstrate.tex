...
\begin{definition}[\(\otimes\)-invertibility of an \(\mathcal{O}_X\)-module]
...
\begin{lemma}[Composite-adjunction homEquiv factorisation]
...
\begin{lemma}[leftAdjointUniq transport of the composite unit]
...
\begin{lemma}[leftAdjointUniq transport of the composite unit ($P$-general form)]
...
\begin{proof}
  ...
  % NOTE (iter-031): the inline 4-step reduction below is now factored into a new abstract Lean helper
  % `deltaConjOfMuComparison` (private, AlgebraicGeometry.Scheme.Modules), which has NO blueprint block
  % yet. The Lean proof of this lemma is a one-line `exact deltaConjOfMuComparison hadj'
  % (pullbackPushforwardAdjunction φ') A B (pushforward_lax_mu_comparison f A B)`. Planner: add a
  % \lean{}-tagged block for the helper and rewrite this proof's prose to \uses it (coverage debt).
...
\begin{lemma}[Pointwise naturality square of the reverse slice transport]
...
\begin{lemma}[Clean pointwise form of the reverse-transport component]
...
\begin{proof}
  ...
  % functorially in $\mathcal{F}$, $\mathcal{G}$.
  % \end{lemma}"
  By \cref{lem:tensorobj_restrict_iso}, \(T(f)\) is assembled by the four-step chart-chase
  \(\mathtt{restrictFunctorIsoPullback}\,f \mathbin{;} \mathtt{sheafificationCompPullback}
  \mathbin{;} \text{(strip)} \mathbin{;} (\mathrm{H1}\circ\mathrm{H2})\). Each ingredient is
  natural in the open immersion: \(\mathtt{restrictFunctorIsoPullback}\) and
  \(\mathtt{sheafificationCompPullback}\) are natural isomorphisms of functors of the immersion,
  and the base-change comparison underlying the strip and \(\mathrm{H1}\circ\mathrm{H2}\) is
  functorial in \(f\) by the Stacks lemma quoted above (pullback commutes with \(\otimes\)
  \emph{functorially}). Restriction along \(j\) is the pullback functor \(\mathtt{restrict}\,j\);
  since \(j \mathbin{;} \iota_U = \iota_V\), the composite
  \(\mathtt{restrict}\,j \circ \mathtt{restrict}\,\iota_U\) is canonically
  \(\mathtt{restrict}\,\iota_V\) (\(\mathtt{restrictFunctorComp}\) / \(\mathtt{pullbackComp}\)).
  Applying \(\mathtt{restrict}\,j\) to \(T(\iota_U)\) and transporting along that composite
  identification therefore yields \(T(\iota_V)\); the identification of the doubly-restricted
  opens with \(V\) is precisely the reindexing \(\varrho\) supplied by
  \cref{lem:image_preimage_of_le}, and the same functoriality gives the right flank as
  \(\varrho \otimes \varrho\). The only content beyond functoriality is threading \(\varrho\).
\end{proof}

\begin{lemma}
[S3: dual-restriction and \(e^M\)-transport commute with further restriction]
  \label{lem:dual_restrict_iso_dualisoofiso_restrict_compat}
  \lean{AlgebraicGeometry.Scheme.Modules.dual_restrict_iso_dualIsoOfIso_restrict_compat}
  \uses{lem:dual_restrict_iso, def:scheme_modules_dual_iso_of_iso, lem:dualisoofiso_trans, lem:image_preimage_of_le}
  With \(j : V \hookrightarrow U\) as above, let \(e^M : L|_{\iota_U} \cong \mathcal{O}_U\) be a
  trivialising datum and write
  \[
    D(\iota_U, e^M) := \mathtt{dual\_restrict\_iso}\,\iota_U\,L \mathbin{;}
      (\mathtt{dualIsoOfIso}\,e^M)^{-1}
      : (\mathtt{dual}\,L)|_{\iota_U} \cong \mathcal{H}om(\mathcal{O}_U, \mathcal{O}_U)
  \]
  (\cref{lem:dual_restrict_iso}, \cref{def:scheme_modules_dual_iso_of_iso}). On the \(V\)-side
  the datum \(e^M\) is refined to \(\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M\), which is by
  construction the \(\mathtt{restrict}\,j\)-image of \(e^M\) (modulo the unit identification
  \(\mathtt{pullbackObjUnitToUnit}\)). Then the constituent commutes with further restriction
  along \(j\): modulo the reindexing \(\varrho\),
  \[
    (\mathtt{restrict}\,j)\,D(\iota_U, e^M)
      \;=\; D\bigl(\iota_V,\,\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M\bigr).
  \]
\end{lemma}
\begin{proof}
  \uses{lem:dual_restrict_iso, def:scheme_modules_dual_iso_of_iso, lem:dualisoofiso_trans, lem:image_preimage_of_le}
  The dual-restriction isomorphism \(\mathtt{dual\_restrict\_iso}\,f\) is the image of a
  structural isomorphism (internal-hom restriction) under \(\mathtt{restrict}\,f\), hence natural
  in the open immersion: \((\mathtt{restrict}\,j)\,\mathtt{dual\_restrict\_iso}\,\iota_U
  = \mathtt{dual\_restrict\_iso}\,\iota_V\) modulo \(\varrho\), by the same
  \(\mathtt{restrictFunctorComp}\) identification used in S2. The transport
  \(\mathtt{dualIsoOfIso}\) is a contravariant functor of its iso argument
  (\cref{lem:dualisoofiso_trans}), and \(\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M\) is, by
  definition, \(e^M\) carried through \(\mathtt{restrict}\,j\) (its construction is
  \(\mathtt{restrictFunctorIsoPullback} \mathbin{;} \mathtt{pullbackCongr} \mathbin{;}
  \mathtt{pullbackComp}^{-1} \mathbin{;} (\mathtt{pullback}\,j)\,e^M \mathbin{;}
  \mathtt{pullbackObjUnitToUnit}\)). Functoriality of \(\mathtt{dualIsoOfIso}\) under this
  identification gives \((\mathtt{restrict}\,j)\,(\mathtt{dualIsoOfIso}\,e^M)^{-1}
  = (\mathtt{dualIsoOfIso}\,(\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M))^{-1}\) modulo
  \(\varrho\). Composing the two legs and threading \(\varrho\) telescopically yields the claim;
  the only content beyond functoriality is the reindexing.
\end{proof}
...
\begin{proof}
  \uses{lem:dual_unit_iso, lem:presheafdualunitiso_naturality, lem:image_preimage_of_le}
  \(\mathtt{dual\_unit\_iso}\) is the sheafification of the presheaf-level unit-duality iso
  \(\mathtt{presheafDualUnitIso}\), itself the sectionwise evaluation-at-\(1\) map of the internal
  hom of the unit into itself; it is the image of a structural isomorphism under
  \(\mathtt{restrict}\,f\) and so is natural in the open immersion. Restriction along \(j\) is a
  functor with \(j \mathbin{;} \iota_U = \iota_V\), so \((\mathtt{restrict}\,j)\) carries the
  \(U\)-instance to the \(V\)-instance modulo the \(\varrho\) reindexing of
  \cref{lem:image_preimage_of_le}. This is the exact pattern already carried out on the unit side
  of this chapter (\cref{lem:presheafdualunitiso_naturality}, the naturality of
  \(\mathtt{presheafDualUnitIso}\) against a unit automorphism), which serves as the proved
  template for the mechanism; here the naturality is taken against the chart morphism \(j\) rather
  than against a unit automorphism. Threading \(\varrho\) is the sole content.
\end{proof}
...
\begin{proof}
  \uses{lem:tensorobj_unit_iso, lem:image_preimage_of_le}
  The left unitor \(\mathtt{tensorObj\_unit\_iso}\) is the image of a structural isomorphism (the
  monoidal left unitor of the substrate) under \(\mathtt{restrict}\,f\), hence natural in the open
  immersion. Restriction along \(j\) is a functor and \(j \mathbin{;} \iota_U = \iota_V\), so
  \((\mathtt{restrict}\,j)\) carries the \(U\)-instance to the \(V\)-instance modulo the
  \(\varrho\) reindexing of \cref{lem:image_preimage_of_le}, exactly as in S2 and S4a. Threading
  \(\varrho\) is the sole content.
\end{proof}
...
\begin{proof}
  \uses{lem:pullback_unit_iso, lem:image_preimage_of_le}
  This fifth constituent carries its own restriction-naturality square. Both
  factors of \(u_\iota(f)\) are natural in the morphism: \(\mathtt{restrictFunctorIsoPullback}\) is
  the natural isomorphism \(\mathtt{restrict}\,f \cong (\mathtt{pullback}\,f)\) of functors of the
  morphism, and \(\mathtt{pullbackUnitIso}\) (\cref{lem:pullback_unit_iso}, ``pullback preserves the
  structure sheaf'') is compatible with the composition law \(\mathtt{pullbackComp}\) of pullbacks.
  Since \(j \mathbin{;} \iota_U = \iota_V\) and \(\mathtt{restrict}\,j\) is a functor, applying
  \(\mathtt{restrict}\,j\) to \(u_\iota(\iota_U)\) and transporting along the
  \(\mathtt{pullbackComp}\) / \(\mathtt{restrictFunctorComp}\) identification yields
  \(u_\iota(\iota_V)\) modulo the \(\varrho\) reindexing of \cref{lem:image_preimage_of_le}; taking
  inverses gives the stated equation. Threading \(\varrho\) is the sole content.
\end{proof}
...
[Trivialisation iso-chain compatible with further restriction (cocycle residual A, step 1)]
  ...
  \uses{lem:tensorobj_restrict_iso_restrict_compat, lem:dual_restrict_iso_dualisoofiso_restrict_compat, lem:dual_unit_iso_restrict_compat, lem:tensorobj_unit_iso_restrict_compat, lem:trivialisation_uiota_restrict_compat, lem:image_preimage_of_le}
...
\end{lemma}
\begin{proof}
  \uses{lem:tensorobj_restrict_iso_restrict_compat, lem:dual_restrict_iso_dualisoofiso_restrict_compat, lem:dual_unit_iso_restrict_compat, lem:tensorobj_unit_iso_restrict_compat, lem:trivialisation_uiota_restrict_compat, lem:tensorobjisoofiso_trans, lem:image_preimage_of_le}
  ...
  (\cref{lem:tensorobj_restrict_iso_restrict_compat}, S2); (2)
  \(\mathtt{dual\_restrict\_iso}\,\iota_U \mathbin{;} (\mathtt{dualIsoOfIso}\,e^M)^{-1}\)
  (\cref{lem:dual_restrict_iso_dualisoofiso_restrict_compat}, S3); (3) \(\mathtt{dual\_unit\_iso}\)
  ...
  (\cref{lem:trivialisation_uiota_restrict_compat}, S4c). Constituents (2)--(3) sit inside the
  bifunctor \(\mathtt{tensorObjIsoOfIso}\,e^M\,(\cdots)\); bifunctoriality
  (\cref{lem:tensorobjisoofiso_trans}) splits it into the \(e^M\)-leg --- whose \(V\)-side
  refinement is the \(\mathtt{restrictIsoUnitOfLE}\,h_{VU}\,e^M = (\mathtt{restrict}\,j)\,e^M\)
  identification --- and the dual-chain leg covered by S3, S4a.
  ...
  this is the asserted equality. (This is the strictly larger analogue of
  \cref{lem:presheafdualunitiso_naturality}, which transports a single trivialising iso
  rather than the full five-constituent contraction.)
