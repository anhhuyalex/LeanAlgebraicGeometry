...
[Pullback commutes with $\otimes$ along an open immersion (K1)]
  ...
  % NOTE: K1 is the sole open ingredient of D4' (lem:pullback_tensor_iso_loctriv). The
  % comparison is invertible for all modules along an open immersion because the pullback
  % there is base change along the structure-sheaf isomorphism, which is strong monoidal.
  % NOTE (iter-021): statement \leanok (scaffold present: STEP A sheafify-reduction +
  % STEP B strong-monoidal witness e, all typecheck). Proof OPEN: sole residual is the
  % `hcompat` mate-compatibility sorry (delta = e.hom), the monoidal-mate compatibility of
  % leftAdjointUniq (delta-side analogue of D2' presheafUnit_comp_map_eta).
...
\begin{proof}
  ...
  strong-monoidal \(\mathtt{pushforward}\,\beta\): this is the monoidal-mate compatibility of
  ...
  the unit pair (D2$'$). Hence \(\delta(\mathtt{pullback}\,\varphi')\) is an isomorphism, so
