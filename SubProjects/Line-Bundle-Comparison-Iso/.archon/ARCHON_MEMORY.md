...
- Top-level extraction of monoidal-carried content FAILS: `MonoidalCategoryStruct (PresheafOfModules X.ringCatSheaf.obj)` not globally synthesizable — keep reductions IN-PROOF (letI/unification). Killed `pullbackTensorMap_restrict_core` iter-018; same wall kills `Functor.Monoidal.transport` for open-immersion δ-iso (K1) — instead mirror closed `tensorObj_restrict_iso` (H1∘H2 in-proof).
- K1 `hcompat` (`leftAdjointOplaxMonoidal hadj .δ = μIsoβ.inv`) = the project-side `hadj.IsMonoidal` instance (sectionwise `homEquiv.injective`+`tensor_ext`), then `laxMonoidalEquivOplaxMonoidal.right_inv`. `IsMonoidal.leftAdjoint_μ` ALONE does NOT fire (μ only at pushed-forward objs). No Mathlib gap. (`recon022`)
