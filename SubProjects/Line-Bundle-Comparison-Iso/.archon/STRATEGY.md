...
|-------|--------|-----------|-----|-------------------|-------|
| Terminal `exists_tensorObj_inverse` (`TensorObjInverse.lean`) | **ACTIVE — B1-crux engine (iter-051)** | ~1–3 | ~100–250 | `sheafificationCompPullback_eq_leftAdjointUniq`/`leftAdjointUniqUnitEta` (root, DONE); `leftAdjointUniq_trans`/`conjugateEquiv_comp`; `pullbackTensorMap_natural`/`_restrict` (project) | **Bridge B2 FULLY CLOSED iter-050** (the 044–049 blocker, gone): 6 axiom-clean lemmas; B2 row moved to Completed. 6 sorries left: B1-crux `H1inv_app_eq_pullbackVal_restrict` (the engine), squares S2/S3/S4a/S4b, `trivialisation_restrict_compat`. progress-critic terminal051: **CONVERGING**. B1-crux crosses the presheaf/sheaf **sheafification boundary** (distinct from B2's clean restrict/pullback world); reduced to an isolated unit-coherence identity, route documented. **HARD STOP (AUTONOMOUS, B1-crux-specific): if B1-crux survives this round with no advance, `effort-break` the unit identity AND/OR `mathlib-analogist` (cross-domain) on the sheafification-intertwines-adjoint-uniqueness shape — NOT user escalation, NOT the `restrictCompReindex`→pullback refactor (that targets B2's conjugate-of-comp, already done).** Risk: dual S3/S4a route (b) unverified. |
...
iter-041. **Current engine = B1-crux `H1inv_app_eq_pullbackVal_restrict`** (sheafification-boundary
unit coherence); the squares ride it + B2.
...
- Dual side S3/S4a: confirm B1's dual analogue (`dual_restrict_iso` vs an internal-Hom map)
  is the same bounded depth as the tensor side before committing the prover there.
