...
private lemma pushforward_lax_mu_comparison_lhs_tmul {X Y : Scheme.{u}} (f : Y ⟶ X)
  ...
  -- PARTIAL (iter-031): split the sectionwise mate composite into the three legs (verified to fire).
  -- The goal is now `(G.map (δ Gβ ≫ (ε⊗ₘε))).app W (unit.app Z .app W (m⊗ₜn)) = μ(ppLax) … (m⊗ₜn)`.
  -- RESIDUAL: the inner unit value `(hadj'.unit.app Z).app W (m⊗ₜn)` — the value lemma
  -- `pushforwardPushforwardAdj_unit_app_app_apply` does NOT fire here via `simp only` (the `hadj'`
  -- let-binding shadows the `pushforwardPushforwardAdj …` form the lemma keys on; the η-side at L4236
  -- worked because its term was not a tensor-`Z`).  Next: expose the `pushforwardPushforwardAdj` form
  -- (un-`let` `hadj'`, or restate via a local `show`/`change`), apply the unit value lemma to reduce the
  -- unit to `Z.map (adj.counit).op`, then the `δ Gβ`/counit legs via the `restrictScalars`-μ family.
  rw [PresheafOfModules.comp_app, ModuleCat.hom_comp, LinearMap.comp_apply]
  -- Collapse the adjunction unit leg to the section restriction map `Z.map (counit.op)` via the
  -- value lemma, built with EXPLICIT arguments (the `let`-bound `hadj'` blocks keyed `rw`/`simp`
  -- matching under `respectTransparency false`; mirror `pushforwardComp_lax_μ`'s `have … ; erw`).
  have hunit := pushforwardPushforwardAdj_unit_app_app_apply
    f.isOpenEmbedding.isOpenMap.adjunction β (Hom.toRingCatSheafHom f).hom
    (by ext U x; exact congr($((f.app_appIso_inv _).symm).hom x))
    (by ext U x; exact congr($(f.appIso_inv_app_presheafMap U.unop) x))
    ((show _root_.PresheafOfModules (X.presheaf ⋙ forget₂ CommRingCat RingCat) from
...
private lemma deltaConjOfMuComparison {C D : Type*} [Category C] [Category D]
