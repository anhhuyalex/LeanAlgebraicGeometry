...
\begin{itemize}
  \item \cref{chap:Albanese_CodimOneExtension} carries an internal conjunction
    ...
    \texttt{lem:ringKrullDimLE_of_coheight_eq_one}, leaving only the
  ...
  \item \cref{chap:RiemannRoch_WeilDivisor}'s order-at-a-prime-divisor map
    ...
    after \texttt{lem:ringKrullDimLE_of_coheight_eq_one} lands, the instance is
...
\texttt{lem:ringKrullDimLE_of_coheight_eq_one} that downstream files consume.
...
\texttt{lem:ringKrullDimLE_of_coheight_eq_one} (and its underlying
...
\begin{itemize}
  \item The right conjunct \(\operatorname{ringKrullDim}(\mathcal{O}_{X,z}) = 1\)
    ...
    pair \(\operatorname{ringKrullDim} \leq 1\) (\texttt{lem:ringKrullDimLE_of_coheight_eq_one})
...
\texttt{lem:ringKrullDimLE_of_coheight_eq_one} synthesises the instance
...
The instance form
\texttt{lem:ringKrullDimLE_of_coheight_eq_one} is the right shape for
...
\begin{enumerate}
  ...
  \item \texttt{AlgebraicGeometry.Scheme.ringKrullDimLE\_of\_coheight\_eq\_one} ---
    \texttt{lem:ringKrullDimLE_of_coheight_eq_one}. Signature (informal):
...
\begin{itemize}
  ...
  \item \textbf{Refactoring \texttt{Scheme.RationalMap.order} to drop its
    ...
    downstream consequence of \texttt{lem:ringKrullDimLE_of_coheight_eq_one}
