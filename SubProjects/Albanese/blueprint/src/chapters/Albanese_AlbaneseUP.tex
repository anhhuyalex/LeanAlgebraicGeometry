...
% sense of \texttt{def:IsAlbanese} of \cref{chap:Jacobian}; together with the
...
T\), taken modulo line bundles pulled back from \(T\) (\texttt{chap:Picard_LineBundlePullback};
...
sense of \texttt{def:JacobianWitness} (\cref{chap:Jacobian}); the remaining
...
\begin{itemize}
  ...
  \item \(\texttt{proper}\): properness of \(\Pic^0_{C/k} \to \Spec \bar k\) from
    ...
    \texttt{thm:Jacobian_proper}'s discussion).
  ...
  \item \(\texttt{geomIrred}\): geometric irreducibility of the identity
    ...
    \texttt{thm:Jacobian_geomIrred}'s discussion).
...
\(k\)-side Albanese witness consumed by \texttt{def:Jacobian}.
...
\begin{itemize}
  \item \textbf{Representability of \(\Pic_{C/k}\) (A.2.c).} Proved in
    ...
    \texttt{thm:fga_pic_representability}; this chapter only consumes it.
  ...
  \item \textbf{The genus-\(0\) case.} The standing hypothesis of this chapter
    ...
    \texttt{def:genusZeroWitness} (\cref{chap:Jacobian}), routed through
    \texttt{prop:rigidity_genus0_curve_to_AV} and the \(\mathbb G_m\)-scaling
