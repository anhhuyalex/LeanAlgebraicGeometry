...
|---|---|---|---|---|---|
| FBC-B — global H⁰ iso (concrete-tilde equalizer chain) | ACTIVE | 3–5 | ~200–400 | `TensorProduct.piRight`, `tensorEqLocusEquiv` (present); Stacks 01XJ (unconfirmed) | (a) per-chart iso done sorry-free; (b) reduced to crux sub-lemma `pullback_spec_tilde_iso_ring_square_natural` under parent `pullback_spec_tilde_iso_restriction_naturality`. Chain decls gain separated/finite/qcoh hyps on fill. 01XJ build (~1–2 iters if absent) folded into estimate; see Open Q. |
| SNAP — `Γ_*(X,L)` graded comm ring assembly | ACTIVE | 2–3 | ~150–300 | **`CategoryTheory.Localization.Monoidal`** (`LocalizedMonoidal`), `DirectSum.GCommSemiring`/`Gmodule` | Coherence via the `LocalizedMonoidal L W ε` synonym (sidesteps the dead `X.Modules` instance diamond). Real work = `W.IsMonoidal` (from `ztensor_whisker_localIso`) + bridging the hand-built morphisms to the synonym's; coherences then literal pentagon/triangle/hexagon. Next round `mathlib-build`. |
...
- SNAP coherence: use Mathlib `CategoryTheory.Localization.Monoidal` (`LocalizedMonoidal L W ε`) —
  ...
  the synonym instantiation/bridges. `β_{𝟙𝟙}=𝟙` = `braiding_tensorUnit_left`+`unitors_equal`.
