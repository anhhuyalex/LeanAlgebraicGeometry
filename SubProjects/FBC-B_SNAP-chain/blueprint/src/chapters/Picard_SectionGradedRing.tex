...
[Object-pinned native form of the localized associator half-assembly]
  ...
  The reassociation-and-associator step of \cref{lem:tensorObjAssoc_hK_lhs}, stated entirely with its
  comparison \(\mu\) object-arguments in \emph{unfolded image form}: every inner presheaf tensor is
  written as \(L'(a\otimes_p b)\) (\(=\texttt{sheafification.obj}(\texttt{tensorObj}\,(C:=\texttt{MonoidalPresheaf}\,X)\,a\,b)\)),
  never as the folded \(\mathcal A.\texttt{tensorObj}\,\mathcal B\). With objects in this form the
  right-association \((W;T);\alpha = W;(T;\alpha)\), the localized associator naturality and
  \texttt{associator\_hom\_app} all match \emph{syntactically}.
\end{lemma}
...
\begin{proof}
  ...
  % timeout, 200000 hb). One documented `sorry` remains in `tensorObjAssoc_hK_lhs_native` (Lean L2219).
  ...
  % another prover round. The chapter convention is statement-`\leanok` only; this caveat is the honest
  % open-proof signal.
\end{proof}
