...
\begin{proof}
  ...
  % helper `tensorObjAssoc_hK_lhs_head` and reduced to a single μ-pair cancel (cancel_epi + keystone
  % `_eq_mu'` + `show`-uniform), but its residual is two defeq-NOT-token-identical `μ_X` (hidden
  % `Localization.fac` proof terms) — the same μ-syntactic-identity wall. hK_lhs-merge and the head-cancel
  % are the SAME wall. Proof block remains unmarked (sorry).
\end{proof}
