...
\begin{proof}
  ...
  \(i_{-,-} = \mu^{-1}\,;\,(c \lotimes c)\) (\cref{def:tensorObjLocalizedIso}) and
  rewrite the segment-1 right-whiskered unit by the canonical keystone
  \cref{lem:whiskeredUnit_eq_localizationMu_canonical}. The localized associator is
  not yet in a form whose \(\mu\)'s are native: its three counit isomorphisms
  \(c_{\mathcal{A}}, c_{\mathcal{B}}, c_{\mathcal{C}}\) must first be pushed through
  the abstract associator \(\alpha_{\mathcal{A},\mathcal{B},\mathcal{C}}\) by
  \texttt{associator\_naturality}
  (\cref{lem:localizationMonoidal_mu_mathlib}), bringing it to the localized
  associator \(\alpha_{L'a,\,L'b,\,L'c}\) on the counit objects; only then does
  \texttt{associator\_hom\_app} (\cref{lem:localizationMonoidal_mu_mathlib}) expand it
  into a native \(\mu\)-string. At that point the comparison
  \(\mu_{\mathcal{A}^{\flat}\otimes_p\mathcal{B}^{\flat},\,\mathcal{C}^{\flat}}\)
  produced by the keystone meets the inverse comparison carried by
  \(i_{\mathcal{A}\otimes\mathcal{B},\mathcal{C}}\). The two comparisons are equal only
  up to reducible definitional unfolding --- they sit on opposite sides of the
  composition boundary between the localized monoidal synonym and the underlying
  sheaf-of-modules category --- so the pair \(\mu^{-1}\,;\,\mu\) is cancelled across
  that boundary up to reducible defeq, peeling off the shared leading comparison first
  so the cancellation lands on the matching pair. The triangle identity
  \(L'(\eta_{\mathcal{P}})\,;\,c_{L'\mathcal{P}} = \mathrm{id}\) collapses the
  remaining unit/counit factor, and \texttt{μ\_natural\_left}
  (\cref{lem:localizationMonoidal_mu_mathlib}) slides the surviving comparison past
  the whiskered counit, leaving the common form \(K\) of \cref{def:assocCommonForm}.
  The boundary-crossing cancellation mechanism is recorded in
