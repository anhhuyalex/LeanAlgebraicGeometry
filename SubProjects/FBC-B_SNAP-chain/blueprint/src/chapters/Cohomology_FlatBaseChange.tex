...
\begin{definition}[Base-change map for the pushforward]
...
\begin{lemma}[Stalk-local criterion for isomorphisms of \(\mathcal{O}_X\)-modules]
...
\begin{lemma}[Basis-local criterion for isomorphisms of \(\mathcal{O}_X\)-modules]
...
\begin{lemma}[Affine-open locality criterion for isomorphisms of \(\mathcal{O}_X\)-modules]
...
\begin{lemma}[Affine pushforward of a tilde-module]
...
\begin{lemma}[Pullback along an isomorphism is an equivalence of module categories]
...
\begin{lemma}[The pullback functor along an isomorphism is an equivalence]
...
\begin{lemma}[Finite affine cover with quasi-compact overlaps]
...
\begin{lemma}[$\Gamma(X,\mathcal{F})$ as a finite sheaf-condition equalizer]
...
\begin{lemma}[Per-chart base-change module isomorphism]
...
\begin{definition}[Chart restriction as an algebra map]
...
\begin{definition}[Base change of the chart restriction map]
...
\begin{lemma}[(b1) Base-change ring square]
...
\begin{lemma}[Functor-level geometric base-change comparison]
...
\begin{lemma}[Geometric base-change comparison of an open immersion]
...
\begin{lemma}[(b2-alg) Module reassociation]
...
\begin{lemma}[Mate of a pullback congruence is the pushforward congruence]
...
\begin{lemma}[Inverse of a pushforward congruence is the reversed congruence]
...
\begin{lemma}[Mate of an extend-scalars \(\mathrm{eqToHom}\) is the restrict-scalars \(\mathrm{eqToHom}\)]
...
\begin{lemma}[Mate-uniqueness by the unit transpose]
...
\begin{lemma}[Per-piece algebraic mate: conjugate of the extend-scalars coherence]
...
[(b2 glue) Abstract mate cocycle of the ring square]
  ...
  % NOTE (iter-026): \lean{} target does NOT yet exist — no `ring_square_cocycle` is formalized.
  % The iter-026 in-file probe (a carrier-free `ring_square_cocycle_probe` + `exact …` fold) REFUTED
  % the "fold is kernel-light" premise below: the `exact` binding heavy carriers to the abstract
  % metavars whnf-timed-out at the fold seam (FlatBaseChange.lean:2235, 200k hb) → build RED. The
  % carrier-free-cocycle escape route is therefore NOT viable as stated; the bomb is at the FOLD, not
  % in the cocycle proof. Route needs structural rethink (see iter/iter-026/review.md).
...
[(b2) Iterated-mate glue of the ring-square naturality]
  ...
  \begin{proof}
    \uses{lem:ring_square_cocycle, lem:ring_square_glue_geom_leg_nat,
          lem:ring_square_glue_alg_leg_nat, lem:pullback_spec_tilde_iso_inv_unit_triangle,
          lem:gammaPushforwardNatIso_comp, lem:pullback_spec_tilde_iso}
    \emph{Mathlib API note.} The mate equivalence is \(\mathrm{TwoSquare}\)-valued:
    \texttt{mateEquiv adj₁ adj₂} maps \(\mathrm{TwoSquare}\,G\,L_1\,L_2\,H\) to
    \(\mathrm{TwoSquare}\,R_1\,H\,G\,R_2\), where \(\mathrm{TwoSquare}\,T\,L\,R\,B\) is (reducibly) the
    type \(T \ggg R \Rightarrow L \ggg B\); recover its underlying natural transformation through
    \texttt{TwoSquare.equivNatTrans} (the \texttt{.natTrans} accessor).
    ...
    \emph{instantiate} that cocycle at the present heavy data by a single
    \[
      \mathtt{exact\ ring\_square\_cocycle\ \dots\ h_{\mathrm{unitTri}}\ h_{\mathrm{geomLeg}}\
        h_{\mathrm{algLeg}}\ h_{\mathrm{comp}}},
    \]
    where the four hypotheses are supplied by the already-closed
    ...
    \cref{lem:gammaPushforwardNatIso_comp} (the composition coherence). Because the conclusion of
    \cref{lem:ring_square_cocycle} is stated over abstract carriers, the \texttt{exact} \emph{binds} the
    heavy carriers \(\widetilde{(-)}\), \(\mathrm{pullback}(\operatorname{Spec}-)\), and
    \(\mathrm{extendScalars}\) to metavariables by unification --- it never \emph{reduces} them --- so no
    carrier weak-head-normal-form is forced and the kernel timeout of the pointwise/concrete route is
    avoided entirely.
    % NOTE (iter-026): this final claim is REFUTED in-file. The iter-026 probe instantiated a
    % carrier-free cocycle at exactly this heavy data via `exact …`; unification DID force carrier whnf
    % to match the concrete goal's structure (the RHS last factor is not syntactically `_.symm`), and
    % it timed out at FlatBaseChange.lean:2235 (whnf, 200k hb) → build RED. The `exact`-binds-to-metavars
    % argument does NOT hold: the goal side must still be whnf'd to unify. The fold, not the cocycle
    % proof, is the bomb. A viable route must make the glue conclusion SYNTACTICALLY a cocycle instance
    % (all four legs in the same shape), or abandon the functor/iso-level glue entirely.
  \end{proof}
\end{lemma}

\begin{lemma}[(b2) Ring-square naturality of the tilde-pullback dictionary]
...
\begin{lemma}[Restriction-naturality of the affine pullback dictionary]
...
\begin{theorem}[Flat base change, $i=0$ case]
...
\begin{definition}[The ground ring of global sections]
...
\begin{definition}[The structure-sheaf restriction ring map \texorpdfstring{$A \to \Gamma(U,\mathcal{O}_X)$}{A to sections over U}]
  \label{def:fbcb_rhoU}
...
\begin{lemma}[Transitivity of the restriction ring maps]
...
\begin{definition}[Sections over \texorpdfstring{$U$}{U} as an \texorpdfstring{$A$}{A}-module]
...
\begin{definition}[The restriction \texorpdfstring{$\Gamma(U,M) \to \Gamma(V,M)$}{Gamma(U,M) to Gamma(V,M)} as an \texorpdfstring{$A$}{A}-module morphism]
...
\begin{definition}[The \texorpdfstring{$A$}{A}-linear restriction map]
...
\begin{lemma}[The \texorpdfstring{$A$}{A}-linear restriction is the underlying presheaf restriction]
...
\begin{lemma}[Functoriality of the \texorpdfstring{$A$}{A}-linear restriction maps]
...
\begin{definition}[The left restriction leg]
...
\begin{definition}[The right restriction leg]
...
\begin{definition}[Restriction of a global section to the cover]
...
\begin{lemma}[The restricted family is compatible]
...
\begin{definition}[The corestriction into the equalizer locus]
...
\begin{lemma}[Global sections as an \texorpdfstring{$A$}{A}-module equalizer locus]
...
\begin{lemma}[Flat base change commutes with the \texorpdfstring{$H^0$}{H0} equalizer]
