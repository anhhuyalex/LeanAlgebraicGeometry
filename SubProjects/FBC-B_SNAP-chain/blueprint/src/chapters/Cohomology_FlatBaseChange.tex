...
[(b2) Ring-square naturality of the tilde-pullback dictionary]
  ...
  \begin{proof}
    \uses{lem:gammaPushforwardNatIso}
    The isomorphism \cref{lem:pullback_spec_tilde_iso} is the adjoint conjugate
    of the \(\Gamma\)-pushforward natural isomorphism
    \cref{lem:gammaPushforwardNatIso}. Hence its naturality across the ring square is precisely the
    naturality of \cref{lem:gammaPushforwardNatIso} under the same square, transported through the
    naturality of the adjoint-conjugation construction. The geometric comparison
    \cref{lem:chartBaseChangeGeometricComparison} (pullback pseudofunctoriality) and the algebraic
    reassociation \cref{lem:chartBaseChangeModuleReassoc} are the two legs of that naturality square;
    both are the image of the single naturality square of the \(\Gamma\)-pushforward isomorphism under
    the ring square, so they coincide. No flatness and no adjoint mate enter.

    \emph{Operational route} (validated to the proof head in iter-004; this is the residual mechanical
    tail, not a new idea). The equation is between isomorphisms, so it suffices to prove the underlying
    \(\hom\)-morphism equality. Rearranging the trailing inverse onto the other side turns the goal into
    an equality of honest morphisms out of the source object \(L_{\mathrm{tot}}.\!obj\,M\) (a double
    pullback of \(\widetilde M\)), where \(L_{\mathrm{tot}}\) is the composite left adjoint
    \(\widetilde{(-)} \ggg \text{pullback}(\operatorname{Spec}\mathrm{inclR}) \ggg
    \text{pullback}(\operatorname{Spec}\rho_B)\). Because maps OUT of \(L_{\mathrm{tot}}.\!obj\,M\)
    transpose without needing any inverse, peel the three adjunctions of \(L_{\mathrm{tot}}\) in turn
    (outermost \(\text{pullback}(\operatorname{Spec}\rho_B)\dashv\text{pushforward}\), then
    \(\text{pullback}(\operatorname{Spec}\mathrm{inclR})\dashv\text{pushforward}\), then
    \(\widetilde{(-)}\dashv\Gamma\)): each peel cancels a common unit prefix and lands the identity one
    level down, finally in \(\mathrm{ModuleCat}\;R\) as a map
    \(M \to \operatorname{restr}_{\mathrm{inclR}}\operatorname{restr}_{\rho_B} N\). There each
    \cref{lem:pullback_spec_tilde_iso} leg transposes to \cref{lem:gammaPushforwardNatIso} by the
    defining unit triangle of the conjugate transform (Mathlib \texttt{unit\_conjugateEquiv}); the
    geometric comparison \cref{lem:chartBaseChangeGeometricComparison} transposes through pushforward
    pseudofunctoriality (Mathlib \texttt{conjugateEquiv\_pullbackComp\_inv}); the reassociation
    \cref{lem:chartBaseChangeModuleReassoc} transposes through \texttt{ModuleCat.extendScalarsComp}.
    All three constituents are identity-on-elements, so the two resulting \(\mathrm{ModuleCat}\;R\)
    maps agree by the pointwise naturality of \cref{lem:gammaPushforwardNatIso} (whose own proof is
