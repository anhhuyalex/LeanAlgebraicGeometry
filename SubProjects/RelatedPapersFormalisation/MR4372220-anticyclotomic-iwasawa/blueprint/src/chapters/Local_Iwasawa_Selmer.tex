...
\section{Local cohomology groups of characters}
...
\section{Selmer groups of characters}
...
\section{Local cohomology groups of \texorpdfstring{\(E\)}{E}}

\begin{lemma}[Local cohomology of \(E\) at \(p\)]
  \label{lem:local-E-cofree}
  \lean{MR4372220.Local.localCohomologyAtP_E}
  \uses{def:iwasawa-algebra, lem:commalg}
  % SOURCE: [MR4372220], Lem.~\ref{lemma411}, §Local cohomology groups of E (read from references/MR4372220-anticyclotomic-iwasawa-source/Eisenstein.tex)
  % SOURCE QUOTE: "Then \(\rH^1(K_w,M_E)\) is \(\Lambda\)-cofree of rank \(2\)."
  \textit{Source: MR4372220, Local cohomology groups of E.}
  Let \(w\) be a prime of \(K\) above \(p\), and assume that \(E(K_w)[p]=0\). Then \(H^1(K_w,M_E)\) is \(\Lambda\)-cofree of rank \(2\).
\end{lemma}

\begin{proof}
The proof is virtually the same as that of \cref{lem:local-char-wp} (the character analogue). Writing \(X:=H^1(K_w,M_E)^\vee\), by \cref{lem:commalg} it suffices to show that \(X[T]=0\) and \(X/TX\) is \(\mathbb{Z}_p\)-free of rank \(2\). The hypothesis \(E(K_w)[p]=0\) implies \(E(K_w)[p^\infty]=0\), so \(H^2(K_w,E[p^\infty])=0\) by local duality. Taking cohomology of \(0\to E[p^\infty]\to M_E\xrightarrow{\times T}M_E\to 0\) gives \(H^1(K_w,M_E)/T\,H^1(K_w,M_E)\simeq H^2(K_w,E[p^\infty])=0\) and \(H^1(K_w,E[p^\infty])\simeq H^1(K_w,M_E)[T]\). The first isomorphism shows \(X[T]=0\); taking cohomology of \(0\to E[p]\to E[p^\infty]\xrightarrow{p}E[p^\infty]\to 0\) shows \(X/TX\simeq H^1(K_w,E[p^\infty])^\vee\) is \(\mathbb{Z}_p\)-free, of rank \(2\) by the local Euler characteristic formula.
\end{proof}
