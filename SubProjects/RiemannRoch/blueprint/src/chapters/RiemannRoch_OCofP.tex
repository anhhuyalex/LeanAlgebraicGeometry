...
\begin{definition}[Carrier set of the line bundle of a closed point over an open]
...
\begin{lemma}[Monotonicity of the carrier set]
...
\begin{definition}[Nonemptiness of the top open of an integral scheme]
...
\begin{definition}[The \(\bar k\)-algebra structure on the function field]
...
\begin{definition}[Carrier submodule of the line bundle of a closed point]
...
\begin{definition}[Bot-trivialisation submodule]
...
\begin{lemma}[Monotonicity of the sheaf-corrected carrier submodule]
...
\begin{definition}[Type-level subfunctor presentation of the carrier]
...
\begin{lemma}[Forward direction of \texttt{globalSections\_iff}: building a
...
\begin{theorem}[The Euler-characteristic bridge \(H^0 - H^1 = 2\) at
\(D = [P]\)]
...
\begin{theorem}[Existence of a non-constant rational function from
\(\dim H^0 = 2\)]
...
\begin{lemma}[Local DVR lift from vanishing order]
...
\begin{lemma}[A finite integral-domain algebra over an algebraically closed
field is the field]
...
\begin{lemma}[Stalk-unit lift from vanishing order at a prime divisor]
