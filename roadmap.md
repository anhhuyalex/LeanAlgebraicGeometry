...
- [~] **Čech higher-direct-image engine (A.2.c)** — `Rⁱf_*` development **merged from `Cech-Cohomology`** (`pushPullFunctor`, `pushPullMap_comp`, `cech_computes_higherDirectImage` proved in the subproject); **9 residual `sorry`** in the AJC tree (`CechSectionIdentificationLeg` ×5, `FlatBaseChange` ×2, `CechToHigherDirectImage`, `CechHigherDirectImageUnconditional`)
